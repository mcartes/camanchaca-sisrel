...
class BitacoraController extends Controller
{
    ...
    public function ListarActividad(Request $request)
    {
        ...
        $comuna_filtro = $request->input('comu_codigo');
        $organizacion_filtro = $request->input('orga_codigo');
        $fecha_inico = $request->input('fecha_inicio');
        $fecha_termino = $request->input('fecha_termino');

        $comunas = Comunas::where('comu_vigente', 'S')->select('comu_codigo', 'comu_nombre')->get();
        $organizaciones = Organizaciones::where('orga_vigente', 'S')->select('orga_nombre', 'orga_codigo')->get();

        $actividades = Actividades::where('actividades.acti_vigente', 'S')
            ->join('organizaciones', 'organizaciones.orga_codigo', 'actividades.orga_codigo');

        if ($comuna_filtro != null) {
            $actividades->join('comunas', 'comunas.comu_codigo', 'actividades.comu_codigo')
                ->where('comunas.comu_codigo', $comuna_filtro);
        }

        if ($organizacion_filtro != null) {
            $actividades->where('organizaciones.orga_codigo', $organizacion_filtro);
        }

        if ($fecha_inico != null && $fecha_termino != null) {
            $fecha_final_add = Carbon::parse($fecha_termino);
            $fecha_final_add = $fecha_final_add->copy()->addDay();
            $actividades->whereBetween('actividades.acti_creado', [$request->fecha_inicio, $fecha_final_add]);
        }
        $actividades = $actividades->get();
        return view('admin.bitacora.listar', compact('actividades', 'comunas', 'organizaciones'));
    }
