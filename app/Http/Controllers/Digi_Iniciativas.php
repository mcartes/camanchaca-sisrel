...
class Digi_Iniciativas extends Controller
{
    ...
    public function show($inic_codigo)
    {
        ...
        if (!$iniciativa)
            return redirect()->route('digitador.iniciativas.index')->with('errorIniciativa', 'La iniciativa seleccionada no se encuentra registrada en el sistema.');
    ...
    public function destroy(Request $request)
    {
        ...
        return redirect()->route('digitador.iniciativas.index')->with('exitoEliminar', 'La iniciativa fue eliminada correctamente.');
    }
    ...
    public function crearEvaluacion($inic_codigo)
    {
        ...
        return view('digitador.iniciativas.evaluacion', [
    ...
    public function guardarEvaluacion(Request $request)
    {
        ...
        $evalCrear = Evaluaciones::create([
            ...
            'eval_vigente' => 'S',
            'eval_rut_mod' => Session::get('digitador')->usua_rut,
            'eval_rol_mod' => Session::get('digitador')->rous_codigo
        ]);
    ...
    public function actualizarEvaluacion(Request $request, $eval_codigo)
    {
        ...
        return redirect()->route('digitador.iniciativas.index')->with('exitoEvaluacion', 'La evaluación de la iniciativa fue actualizada correctamente.');
    }
    ...
    public function eliminarEvaluacion($eval_codigo)
    {
        ...
        return redirect()->route('digitador.iniciativas.index')->with('exitoEvaluacion', 'La evaluación de la iniciativa fue eliminada correctamente.');
    }
    ...
    public function completarCobertura($inic_codigo)
    {
        ...
        $partObtener = DB::table('participantes')
            ...
            ->where('inic_codigo', $inic_codigo)
            ->get();
        return view('digitador.iniciativas.cobertura', [
            'iniciativa' => $inicObtener,
    ...
    public function actualizarCobertura(Request $request, $inic_codigo)
    {
        ...
        foreach ($subeCodigos as $sube_codigo => $cantidad) {
            if ($sube_codigo != '_token' && $sube_codigo != 'inic_codigo') {
                $partActualizar = Participantes::where(['inic_codigo' => $inic_codigo, 'sube_codigo' => $sube_codigo])->update([
                    ...
                    'part_vigente' => 'N',
                    'part_rut_mod' => Session::get('digitador')->usua_rut,
                    'part_rol_mod' => Session::get('digitador')->rous_codigo
                ]);
                if ($partActualizar)
        ...
        if ($actualizados != (count($request->all()) - 2))
            return redirect()->route('digitador.cobertura.index', $inic_codigo)->with('errorCobertura', 'Algunos registros no fueron actualizados correctamente.');
    ...
    public function completarResultados($inic_codigo)
    {
        ...
        $resuObtener = DB::table('resultados')
            ...
            ->where('inic_codigo', $inic_codigo)
            ->get();
        return view('digitador.iniciativas.resultados', [
            'iniciativa' => $inicObtener,
    ...
    public function actualizarResultados(Request $request, $inic_codigo)
    {
        ...
        foreach ($resuCodigos as $resu_codigo => $cantidad) {
            if ($resu_codigo != '_token' && $resu_codigo != 'inic_codigo') {
                $resuActualizar = Resultados::where(['inic_codigo' => $inic_codigo, 'resu_codigo' => $resu_codigo])->update([
                    ...
                    'resu_rut_mod' => Session::get('digitador')->usua_rut,
                    'resu_rol_mod' => Session::get('digitador')->rous_codigo
                ]);
                if ($resuActualizar)
                    $actualizados = $actualizados + 1;
            }
        ...
        if ($actualizados != (count($request->all()) - 2))
            return redirect()->route('digitador.resultados.index', $inic_codigo)->with('errorResultados', 'Algunos registros no fueron actualizados correctamente.');
    ...
    public function listarEvidencia($inic_codigo)
    {
        ...
        if (!$inicVerificar)
            return redirect()->route('digitador.iniciativas.index')->with('errorIniciativa', 'La iniciativa no se encuentra registrada en el sistema.');
    ...
    public function guardarEvidencia(Request $request, $inic_codigo)
    {
        try {
            ...
            if ($validarEntradas->fails())
                return redirect()->route('digitador.evidencia.listar', $inic_codigo)->with('errorValidacion', $validarEntradas->errors()->first());
            ...
            $inevGuardar = IniciativasEvidencias::insertGetId([
                ...
                'inev_vigente' => 'S',
                'inev_rol_mod' => Session::get('digitador')->rous_codigo,
                'inev_rut_mod' => Session::get('digitador')->usua_rut
            ]);
            if (!$inevGuardar)
            ...
            if (!$inevActualizar)
                return redirect()->back()->with('errorEvidencia', 'Ocurrió un error al registrar la evidencia, intente más tarde.');
            return redirect()->route('digitador.evidencia.listar', $inic_codigo)->with('exitoEvidencia', 'La evidencia fue registrada correctamente.');
        } catch (\Throwable $th) {
            return redirect()->route('digitador.evidencia.listar', $inic_codigo)->with('errorEvidencia', 'Ocurrió un problema al registrar la evidencia, intente más tarde.');
        }
    }
    ...
    public function actualizarEvidencia(Request $request, $inev_codigo)
    {
        try {
            ...
            if ($validarEntradas->fails())
                return redirect()->route('digitador.evidencia.listar', $evidencia->inic_codigo)->with('errorValidacion', $validarEntradas->errors()->first());
            ...
            return redirect()->route('digitador.evidencia.listar', $evidencia->inic_codigo)->with('exitoEvidencia', 'La evidencia fue actualizada correctamente.');
        } catch (\Throwable $th) {
    ...
    public function eliminarEvidencia($inev_codigo)
    {
        try {
            ...
            return redirect()->route('digitador.evidencia.listar', $evidencia->inic_codigo)->with('exitoEvidencia', 'La evidencia fue eliminada correctamente.');
        } catch (\Throwable $th) {
    ...
    public function crearPaso1()
    {
        ...
        return view('digitador.iniciativas.paso1', [
    ...
    public function verificarPaso1(Request $request)
    {
        ...
        foreach ($unidCodigos as $codigo) {
            array_push($inunDatos, [
                ...
                'inun_vigente' => 'S',
                'inun_rut_mod' => Session::get('digitador')->usua_rut,
    ...
    public function editarPaso1($inic_codigo)
    {
        ...
        return view('digitador.iniciativas.paso1', [
            ...
            'cargos' => $listarCargos,
            'mecanismos' => $listarMecanismo,
            'submecanismos' => $listarSubmecanismos,
            'frecuencias' => $listarFrecuencias
        ]);
    }
    ...
    public function actualizarPaso1(Request $request, $inic_codigo)
    {
        $request->validate(
            ...
            [
                ...
                'nombreresponsable.max' => 'El nombre del encargado responsable excede el máximo de caracteres permitidos (100).',
                'submecanismo.required' => 'La actividad asociada es requerida.',
                'mecanismo.required' => 'Es requerido que se asigne un mecanismo',
                'frecuencia.required' => 'La frecuencia es requerida.'
            ]
        );
    ...
    public function editarPaso2($inic_codigo)
    {
        ...
        return view('digitador.iniciativas.paso2', [
    ...
    public function actualizarPaso2(Request $request, $inic_codigo)
    {
        ...
        foreach ($impaCodigos as $impacto) {
            array_push($inimDatos, [
                ...
                'inim_vigente' => 'S',
                'inim_rut_mod' => Session::get('digitador')->usua_rut,
    ...
    public function editarPaso3($inic_codigo)
    {
        ...
        $listarParticipantes = DB::table('participantes')
            ...
            ->orderBy('part_creado', 'asc')
            ->get();
        return view('digitador.iniciativas.paso3', [
    ...
    public function guardarUbicacion(Request $request)
    {
        ...
        $inubCrear = IniciativasUbicaciones::create([
            ...
            'inub_vigente' => 'S',
            'inub_rut_mod' => Session::get('digitador')->usua_rut,
            'inub_rol_mod' => Session::get('digitador')->rous_codigo
        ]);
        if (!$inubCrear)
    ...
    public function guardarDinero(Request $request)
    {
        ...
        if (!$codiVerificar) {
            $codiGuardar = CostosDinero::create([
                ...
                'codi_vigente' => 'S',
                'codi_rut_mod' => Session::get('digitador')->usua_rut,
                'codi_rol_mod' => Session::get('digitador')->rous_codigo
            ]);
        } else {
    ...
    public function guardarEspecie(Request $request)
    {
        ...
        $coesGuardar = CostosEspecies::create([
            ...
            'coes_vigente' => 'S',
            'coes_rut_mod' => Session::get('digitador')->usua_rut,
    ...
    public function guardarInfraestructura(Request $request)
    {
        ...
        $coinGuardar = CostosInfraestructura::create([
            ...
            'coin_vigente' => 'S',
            'coin_rut_mod' => Session::get('digitador')->usua_rut,
            'coin_rol_mod' => Session::get('digitador')->rous_codigo
        ]);
        if (!$coinGuardar)
    ...
    public function guardarRrhh(Request $request)
    {
        ...
        $corhGuardar = CostosRrhh::create([
            ...
            'corh_vigente' => 'S',
            'corh_rut_mod' => Session::get('digitador')->usua_rut,
            'corh_rol_mod' => Session::get('digitador')->rous_codigo
        ]);
        if (!$corhGuardar)
