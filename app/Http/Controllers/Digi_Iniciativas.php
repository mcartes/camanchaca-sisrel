...
class Digi_Iniciativas extends Controller {
    ...
    public function show($inic_codigo) {
        ...
        if (!$iniciativa) return redirect()->route('digitador.iniciativas.index')->with('errorIniciativa', 'La iniciativa seleccionada no se encuentra registrada en el sistema.');
    ...
    public function guardarEvidencia(Request $request, $inic_codigo) {
        try {
            ...
            if ($validarEntradas->fails()) return redirect()->route('digitador.evidencia.listar', $inic_codigo)->with('errorValidacion', $validarEntradas->errors()->first());
